...
\doi{} % DOI will be assigned by IEEE Access production
...
\begin{abstract}
Multi-agent large language model (LLM) systems are increasingly used for software engineering tasks, yet the role of \textbf{communication architecture} in these systems remains poorly understood. Most existing approaches assume message passing, where agents exchange sequential natural-language summaries, without examining the information loss this assumption may introduce. This paper makes a methodological and diagnostic contribution to that question. We present \textbf{SE-Blackboard}, a modular framework that places message passing, a shared-state blackboard, and a hybrid design within an otherwise identical multi-agent pipeline, and we introduce the \textbf{Information Fidelity Score (IFS)}, a stage-level metric that measures how well key technical entities from the original issue survive successive agent handoffs. On 50 SWE-bench Lite issues, the blackboard configuration improves Coder-stage IFS by 62\% over message passing (0.586 vs. 0.362) and correct file targeting by 27.5 percentage points (82.6\% vs. 55.1\%). Once the correct file is identified, however, all configurations converge to a $\sim$22\% conditional resolve rate, revealing a downstream bottleneck in LLM diff generation. Building on these decompositions, we present an \textbf{information-flow bottleneck model} that predicts how upstream gains attenuate at the patch-generation stage; the model accounts for the modest end-to-end resolve-rate improvement observed (16\% vs. 12\%). These findings identify communication architecture as a measurable, separable design dimension and supply tools---IFS and the bottleneck model---for diagnosing where future improvements in multi-agent SE pipelines should be targeted.
\end{abstract}
...
For automated bug fixing, AgentCoder \cite{huang2024agentcoder} and CodeR \cite{chen2024coder} use multi-agent pipelines that incorporate iterative feedback from test execution, whereas Agentless \cite{xia2024agentless} shows that carefully designed prompting strategies can attain comparable results without deploying multiple agents. More recently, MAGIS \cite{tao2024magis} proposes an LLM-based multi-agent framework specifically for GitHub issue resolution, organized around role-specialized agents that exchange information through structured handoffs; its design goals are closely aligned with ours, though it does not isolate the communication architecture as an independent variable. This comparison suggests that interaction design may be as influential as agent multiplicity itself.

Recent advances also extend agent capability through tool integration: SWE-Agent \cite{yang2024sweagent} formalizes an agent--computer interface that exposes repository navigation and file editing as discrete tools, AutoCodeRover \cite{zhang2024autocoderover} couples spectrum-based code search with iterative LLM reasoning, and Agentless \cite{xia2024agentless} achieves comparable results using only carefully structured prompts. These developments share a common emphasis on what each agent can \emph{do} (search, read, execute, retry) rather than how multiple agents \emph{exchange} information among themselves.
...
A key design principle is \textbf{field ownership}. Each agent is only allowed to write to its designated state fields and has read access to upstream fields. Formally, the write-permission mapping $W : \mathcal{K} \to 2^{\{I,A,P,R,T\}}$ assigns each agent role $k \in \mathcal{K} = \{\text{Planner}, \text{Coder}, \text{Reviewer}, \text{Tester}\}$ the subset of state fields that role is permitted to modify. It is defined as follows:
...
The IssueInfo field $I$ is excluded from every $W(k)$, making it read-only for all agents. This ownership model prevents agents from inadvertently overwriting each other's contributions and ensures a clear audit trail. The combination of a structured shared state with role-bound write permissions is consistent with recent cognitive-architecture treatments of language agents~\cite{sumers2024cognitive}, which advocate explicit separation between working memory, reasoning state, and action production---a separation our $\mathbf{S} = (I, A, P, R, T)$ schema realizes at the granularity of pipeline stages.
...
We select a stratified sample of 50 issues to balance evaluation cost, repository diversity, and difficulty coverage while remaining comparable to prior small-scale SWE-bench Lite studies. Issues were initially sampled in equal strata based on repository metadata to avoid concentration on any single project, then reclassified post-hoc using gold patch complexity as an objective difficulty proxy: Easy (single-file, $<$20 lines): 22 issues, predominantly Django; Medium (single-file, 20--50 lines or multi-file with small changes): 5 issues; Hard (multi-file, $>$50 lines, or complex reasoning): 23 issues. The final sample covers 8 of the 12 repositories in SWE-bench Lite, with Django overrepresented (22/50) because its comprehensive test coverage makes a larger fraction of issues passable under the FAIL\_TO\_PASS / PASS\_TO\_PASS evaluation protocol; we discuss the implications of this concentration for external validity in Section~\ref{sec:threats}.
...
\textbf{Relationship to prior multi-agent evaluation criteria.} IFS differs in scope from common multi-agent evaluation criteria, which include end-to-end task success (e.g., resolve rate), role-consistency metrics, and dialogue-quality scores. Task-success metrics summarize the whole pipeline into a single binary outcome and therefore conflate communication quality with downstream generation capability; role-consistency and dialogue-quality metrics target generic conversational behavior and do not test whether domain-specific technical entities (function names, file paths, error types) survive each handoff. IFS instead measures stage-level preservation of those technical entities and is therefore designed to isolate the contribution of the communication layer from confounds such as patch-formatting reliability. We use IFS alongside, not in place of, resolve rate.
...
\begin{tabular}{lcccc}
\toprule
\textbf{Comm.} & \textbf{Res.} & \textbf{Rate} & \textbf{Tokens} & \textbf{Latency} \\
\midrule
MP & 6/50 & 12.0\% & 25,316 & 106s \\
BB & 8/50 & 16.0\% & 54,849 & 171s \\
Hybrid & 6/50 & 12.0\% & 43,278 & 128s \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table*}[!ht]
\centering
\caption{Information Fidelity Score by pipeline stage (mean $\pm$ std).}
\label{tab:ifs}
\begin{tabular}{lcccc}
\toprule
\textbf{Comm.} & \textbf{Planner} & \textbf{Coder} & \textbf{Reviewer} & \textbf{Tester} \\
\midrule
MP & 0.749$^{\dagger}$ & 0.362 $\pm$ 0.385 & N/A & N/A \\
BB & 0.749 $\pm$ 0.308 & 0.586 $\pm$ 0.342 & 0.454 $\pm$ 0.361 & 0.074 $\pm$ 0.165 \\
Hybrid & 0.738 $\pm$ 0.332 & 0.539 $\pm$ 0.357 & 0.503 $\pm$ 0.340 & 0.079 $\pm$ 0.246 \\
\bottomrule
\end{tabular}

\vspace{1mm}
\footnotesize
$^{\dagger}$Planner IFS for MP is identical to that for BB by design: the Planner is invoked with the same prompt and the same input (the original issue $I$) in all configurations, and the LLM is run with temperature 0 for deterministic generation. The intermediate text output of the MP Planner was not separately persisted; we therefore report the BB Planner-stage value, which is the value the MP Planner would produce in this experimental setting. Standard deviations are computed across the per-issue IFS values available for that stage.
\end{table*}
...
For BB and Hybrid, IFS decays across the pipeline stages (Figure~\ref{fig:ifs-analysis}b): Planner ($\sim$0.74) $\rightarrow$ Coder ($\sim$0.56) $\rightarrow$ Reviewer ($\sim$0.48) $\rightarrow$ Tester ($\sim$0.08). This decline reflects a shift toward implementation tasks in downstream stages rather than reproducing terminology from the original issue description. A manual inspection of a sample of Tester outputs confirms that they primarily report pass/fail counts, the names of the FAIL\_TO\_PASS tests that were executed, and SWE-bench harness messages, while they do not generally reproduce the function or class identifiers from the original issue description; this is consistent with the low Tester IFS and indicates that Tester IFS is not a useful diagnostic for upstream information preservation.
...
\subsubsection{Per-Repository Breakdown}
\label{sec:per-repo-ifs}

A concern that arises from the sample's repository composition is whether BB's IFS advantage might be driven by the over-representation of Django (22/50 issues). To address this, we recompute Coder-stage IFS separately for each repository in the sample (Table~\ref{tab:per-repo-ifs}). The Coder-IFS advantage of BB over MP holds in \emph{every} repository: from a modest $+17$\% on Django (where MP already retains comparatively rich content) up to substantially larger gains on sympy ($+178$\%), astropy ($+187$\%), sphinx-doc ($+79$\%), and pytest-dev (an effective $33\times$ improvement on issues where MP retained almost none of the original technical entities). The corresponding correct-file-targeting rates also rise under BB across every repository. The resolve-rate advantage, in contrast, is observed primarily on easy Django issues, consistent with the bottleneck model (Section~\ref{sec:resolve-rate-limited}): on the harder sympy and astropy issues the downstream patch-generation bottleneck dominates, so upstream gains do not translate into additional resolved issues. The IFS effect of the Blackboard architecture is therefore an architecture-level property of the communication layer rather than a Django-specific artifact.

\begin{table*}[!ht]
\centering
\caption{Per-repository Coder-stage IFS and correct file targeting (non-empty patches). Number in parentheses is the count of available IFS measurements.}
\label{tab:per-repo-ifs}
\begin{tabular}{llcccc}
\toprule
\textbf{Repo} & \textbf{N} & \textbf{Comm.} & \textbf{Resolved} & \textbf{Coder IFS} & \textbf{File targeting} \\
\midrule
django      & 22 & MP & 4/22 & 0.474 & 68.2\% \\
            &    & BB & 7/22 & \textbf{0.555} & \textbf{76.2\%} \\
            &    & Hybrid & 5/22 & 0.541 & 72.7\% \\
\midrule
sympy       & 16 & MP & 0/16 & 0.207 & 20.0\% \\
            &    & BB & 0/16 & \textbf{0.577} & \textbf{76.9\%} \\
            &    & Hybrid & 0/16 & 0.583 & 75.0\% \\
\midrule
sphinx-doc  & 3  & MP & 1/3  & 0.472 & 66.7\% \\
            &    & BB & 0/3  & \textbf{0.847} & \textbf{100.0\%} \\
            &    & Hybrid & 0/3  & 0.875 & 100.0\% \\
\midrule
pytest-dev  & 2  & MP & 1/2  & 0.016 & 100.0\% \\
            &    & BB & 1/2  & \textbf{0.532} & 100.0\% \\
            &    & Hybrid & 1/2  & 0.032 & 100.0\% \\
\midrule
astropy     & 2  & MP & 0/2  & 0.147 & 50.0\% \\
            &    & BB & 0/2  & \textbf{0.423} & \textbf{100.0\%} \\
\midrule
...
\subsubsection{Robustness of IFS to Matcher Choice}
\label{sec:ifs-robustness}

Because IFS relies on automated entity matching, we examine whether its results depend on the matcher implementation. We re-evaluated IFS on a 19-issue subset using a stricter fuzzy matcher based on Python's \texttt{difflib.SequenceMatcher} with a partial-match threshold of $0.80$. On this subset the rule-based matcher yields a BB-over-MP relative IFS gap of $+39.3$\%; the fuzzy matcher yields $+30.6$\%. Both matchers preserve the directional advantage of BB. The relative magnitude is somewhat sensitive to matcher strictness---as expected, because the rule-based matcher counts any case-insensitive substring hit while the difflib matcher requires consecutive-token similarity above $0.80$---but the qualitative finding that BB preserves more upstream information than MP is robust to this measurement choice.
...
To characterize this failure mode concretely, we inspected each of the 18 Hybrid empty-patch runs. In every one of them, the Coder produced no parseable patch object at all: \texttt{blackboard\_final\_state.patches} is empty, even though the Reviewer was invoked three times as expected. This is not the same failure mode as a malformed or partially-correct diff; rather, the Coder appears to abandon the patch-emission step entirely when given the combined message-passing summary plus the full structured blackboard state. The same 18 issues are also unresolved under MP and BB, so the failures concentrate on intrinsically hard issues; Hybrid's dual-channel context appears to suppress the Coder's output on precisely the issues where the model already has the least useful signal. This corroborates the ``context overload'' hypothesis~\cite{liu2024lost} with direct, in-experiment evidence and suggests that future hybrid designs should constrain the Coder's input rather than offer both channels simultaneously.
...
\subsubsection{Reviewer Accept/Reject Dynamics}
\label{sec:reviewer-dynamics}

The Reviewer agent decides whether the Coder's current patch should be accepted, returned for revision (rejected), or sent forward to the Tester at the iteration limit (3 by default). Aggregating Reviewer verdicts from \texttt{blackboard\_final\_state.reviews} across all 50 issues exposes a clear behavioral difference between BB and Hybrid. Under BB, the Reviewer issues 55 accept and 80 reject verdicts; first acceptance occurs on iteration 1 in 21 runs, on iteration 2 in 4, on iteration 3 in 8, and never in 17 runs. Under Hybrid, the corresponding numbers are 37 accept, 103 reject, with first acceptance on iteration 1 in only 13 runs and never-accepted in 29 runs. BB therefore both elicits more first-iteration acceptances and reduces the number of runs in which the Reviewer never accepts a patch, despite the loop being bounded at the same three iterations in all configurations. The MP runs do not persist explicit verdicts, but their distribution over iteration counts (5/1/44 for iters=1/2/3) is closer to Hybrid than to BB, again consistent with BB's higher-quality patches being accepted earlier. These results indicate that BB's information-preservation benefit propagates downstream into measurably more efficient Reviewer behavior, even though the resulting end-to-end resolve rate gain is modest because of the diff-generation bottleneck.
...
To examine whether tool access can address the patch-generation bottleneck, we conducted an ablation study inspired by the Toolformer paradigm \cite{schick2023toolformer}. In this setup, the Coder is equipped with five tools: \texttt{read\_file} (with optional line range), \texttt{search\_code} (substring search across the working tree), \texttt{read\_lines} (read a specified line range from a previously discovered file), \texttt{validate\_patch} (dry-run apply via \texttt{git apply --check}), and \texttt{submit\_patch} (final answer). The tools are exposed via a JSON schema, and the Coder is instructed to call \texttt{read\_file} or \texttt{search\_code} at least once before producing a diff, to use \texttt{validate\_patch} prior to \texttt{submit\_patch}, and to retry up to two times if validation fails before submitting anyway. The system prompt and tool descriptions are otherwise identical to the non-tool baseline; all other pipeline components (Planner, Reviewer, Tester) are unchanged. We evaluated 10 issues under both MP and BB (Table~\ref{tab:tool-ablation}).
...
Tool use does not improve resolve rates for either configuration. Although the \texttt{validate\_patch} tool detects format errors, the LLM cannot correct them even after multiple retries. Token consumption increases 2--4x without yielding additional resolutions. These results confirm that the bottleneck is in the LLM's intrinsic diff-generation ability, rather than information insufficiency or lack of tool access. We note that this ablation uses 10 issues per configuration, which is sufficient to rule out a large positive effect but not to establish a precise null; we therefore treat the result as directional evidence consistent with the bottleneck model rather than as a definitive negative finding.

\subsection{Cost-Efficiency}

BB incurs significantly higher token costs: 54,849 tokens per issue versus MP's 25,316 (2.17x, Wilcoxon $W=34$, $p<0.001$). Latency shows a similar pattern (171s vs. 106s, 1.61x, Wilcoxon $W=63$, $p<0.001$). To localize the overhead, we decompose token consumption per agent role (Table~\ref{tab:cost-per-stage}). The Coder consumes a comparable number of tokens in all three configurations ($\sim$17--20k), and the Planner is also broadly comparable ($\sim$1.5--4.4k). The dominant overhead under BB and Hybrid is concentrated at the Reviewer (28.2k for BB, 19.3k for Hybrid versus 3.5k for MP), because the Reviewer reads the full serialized blackboard state at each iteration. The Tester also incurs noticeably more tokens under BB and Hybrid (3.4k and 2.9k versus 0.6k under MP) for the same reason.

\begin{table}[!ht]
\centering
\small
\caption{Mean tokens consumed per agent role and per run, summed across iterations.}
\label{tab:cost-per-stage}
\begin{tabular}{lcccc}
\toprule
\textbf{Comm.} & \textbf{Planner} & \textbf{Coder} & \textbf{Reviewer} & \textbf{Tester} \\
\midrule
...
When measured in tokens per resolved issue---arguably the more practically relevant cost figure than tokens per run---MP requires approximately 211{,}000 tokens, BB about 343{,}000 tokens, and Hybrid about 361{,}000 tokens. BB therefore spends roughly $1.6\times$ as many tokens per resolution as MP, even though it resolves $1.33\times$ as many issues. Practitioners considering BB at present resolve rates should regard this as a deliberate trade of fixed serialization overhead for higher upstream information quality, rather than as a free improvement. We also note that tokens consumed in successful runs are substantially lower than in failed runs in every configuration (MP: 13.4k vs.\ 26.9k; BB: 29.2k vs.\ 59.7k; Hybrid: 31.8k vs.\ 44.8k), reflecting that successful runs typically terminate after one or two Coder--Reviewer iterations, while failures hit the iteration ceiling. The serialization overhead of BB would therefore be amortized to an increasingly favorable ratio if the downstream patch-generation bottleneck (Section~\ref{sec:resolve-rate-limited}) is addressed and the resolve rate rises.
...
\textbf{Concrete Impact}: MP consumed 34,042 tokens across 3 iterations (131s) and failed; BB consumed 30,732 tokens in 1 iteration (67s) and succeeded.

\textbf{Path-wise comparison.} Tracing the agent interaction sequences makes the divergence explicit:
\begin{itemize}
\item \emph{MP path}: Issue $\xrightarrow{C^{\text{MP}}(\text{Planner})=\{I\}}$ Planner produces a 679-token summary that drops the file/line traceback $\rightarrow$ Coder receives only this summary ($C^{\text{MP}}(\text{Coder})=\{\text{out}(\text{Planner})\}$) and incorrectly patches \texttt{Query.clone()} (line 355) $\rightarrow$ Reviewer rejects $\rightarrow$ Coder re-attempts twice on a similarly wrong target $\rightarrow$ Tester fails (3 iterations).
\item \emph{BB path}: Issue $\xrightarrow{C^{\text{BB}}(\text{Planner})=\{I\}}$ Planner produces the same diagnosis $\rightarrow$ Coder receives the structured analysis \emph{and} the original issue text including the traceback ($C^{\text{BB}}(\text{Coder})=\{A, I\}$) $\rightarrow$ Coder targets \texttt{Query.check\_filterable} on the first try $\rightarrow$ Reviewer accepts $\rightarrow$ Tester passes (1 iteration).
\end{itemize}
The divergence between the two paths occurs precisely at the Planner $\rightarrow$ Coder handoff, where MP's lossy natural-language compression discards the file/line information that survives intact in BB's shared state. This example illustrates how message passing can introduce information loss during summarization: even when the Planner's diagnosis is correct, the compressed representation may omit the precise location details required for accurate patch generation.
...
\item \textbf{Downstream: A systematic bottleneck limits all modes.} Once the correct file is identified, all modes achieve $\sim$22\% conditional resolve rate. We attribute this convergence to the LLM's difficulty in reliably generating context-correct unified diffs---a property we observe empirically across all 50 issues and all three configurations rather than infer from prior work: the $\sim$78\% of correct-file patches that nevertheless fail to apply or to pass tests do so primarily because of mismatched context lines, incorrect line numbers in hunk headers, or off-by-one offsets in the generated diffs (see also Section~\ref{sec:tool-ablation-study}, where giving the Coder direct file-read and validate tools does not improve this rate).
...
\textbf{A note on statistical power.} A post-hoc power analysis indicates that detecting a 4 percentage-point difference (12\% vs. 16\%) using McNemar's test at $\alpha = 0.05$ with 80\% power would require approximately $\mathbf{N = 300}$ paired observations, roughly six times the size of our sample. Because most pipeline runs share a downstream bottleneck (Section~\ref{sec:patch-quality-analysis}), discordant pairs between MP and BB are intrinsically scarce ($b + c = 4$ in our experiment), which compounds the power problem on this dataset: even a doubled sample at the present discordance rate would still leave the binary outcome under-powered. Accordingly, this study was not designed to establish statistical significance for resolve rate alone, and the 4pp resolve-rate improvement should be interpreted as descriptive rather than confirmatory. Resolve rate is interpreted as one component of a multi-metric evaluation strategy, where IFS ($p = 0.058$), file targeting (+27.5pp), the bottleneck model, the tool-use ablation, and the case study together support a consistent explanation. We view this convergence of evidence as more informative than any single p-value.
...
However, among non-empty patches, Hybrid achieves a file-targeting rate of 78.1\%, close to BB's 82.6\%, and its conditional resolve rate (24.0\%) is the highest among the three configurations. These results suggest that the limitation arises from input format and context size, rather than insufficient information quality. Therefore, future hybrid designs may benefit from mechanisms that limit context overload, such as selective sharing, progressive disclosure, or memory-management techniques in which the relevant subset of the shared state is surfaced to the Coder on demand rather than as a single combined context~\cite{zhong2024memorybank}.
...
\textbf{Alternative Patch Representations.} The unified diff bottleneck is not resolved through tool augmentation alone. Future work could explore patch representations that avoid strict context-line matching. Three concrete directions are most promising: (i) \emph{search-and-replace specifications}, where the Coder emits a list of \texttt{(old\_block, new\_block)} pairs and a deterministic tool performs the substitution---this avoids context-line matching entirely while keeping outputs short; (ii) \emph{AST-level edits}, where the Coder produces structured tree transformations applied by a language-aware engine---this guarantees syntactic validity but requires per-language tooling; and (iii) \emph{whole-file rewriting}, where the Coder emits the full content of the modified file and the diff is recovered by external comparison---this scales poorly to large files but eliminates the diff-generation step at the model level.

\textbf{Improved Diff Generation.} Improving diff generation itself is another promising direction. Potential approaches include fine-tuning on patch corpora, constrained decoding for diff syntax, deterministic post-hoc repair of corrupt hunk headers, or retrieval-augmented generation \cite{lewis2020retrieval} to retrieve accurate context lines while retaining the unified diff format. A practical compromise---used outside this study---is to apply patches with progressively more lenient apply heuristics (\texttt{git apply} $\rightarrow$ \texttt{patch -F0} $\rightarrow$ \texttt{patch -F3}) so that minor context errors do not cause outright failure; this trades a small risk of incorrect application against substantially higher apply rates.
